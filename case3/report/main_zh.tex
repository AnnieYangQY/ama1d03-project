...
\textbf{主要发现：}在相同基金下比较起始年龄，20岁与40岁（全球/香港基金）的替代率相差\textbf{27个百分点}（36\% 对 9.1\%）。Peter 在20岁时即使选择表现较差的香港基金（17.5\%），最终余额仍远高于 Mary。\textbf{供款年期比基金选择更为重要。}
...
Mary 迫切需要大量 TVC 或其他收入来源。Peter 要达到 60\% 替代率，每月仍需约 HK\$1,600 额外供款，若按年规划可在 TVC 扣税上限内实现。
...
\textbf{Peter（2045年退休）：}
\begin{itemize}
  \item 强积金提供良好基础（替代率 36\%）。
  \item 每月增加约 HK\$1,600 TVC，可使强积金单独达到 60\% 替代率。
...
\textbf{Mary（2025年退休）：}
\begin{itemize}
  \item 单靠强制强积金严重不足（替代率 9.1\%）。
  \item 即使领取高额 OALA（综合替代率 22.5\%），距离 60\% 仍有较大缺口。
  \item 应尽快开始 TVC；可考虑延迟至65岁后继续工作。
  \item 鉴于复利时间有限，须建立第三支柱储蓄。
\end{itemize}

Mary 的现实综合方案——强积金（HK\$2,836）+ 高额 OALA（HK\$4,185）+ 兼职或储蓄（HK\$5,000）——每月约 HK\$11,021，仍低于 60\% 目标（HK\$18,706）。
